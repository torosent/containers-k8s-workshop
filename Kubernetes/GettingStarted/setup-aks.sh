...
# without pre-generated SSH keys and SP
#az aks create --resource-group myK8sCluster --name myK8sCluster --node-vm-size Standard_DS3_v2 --node-count 2 --generate-ssh-keys -k 1.9.2

# with SSH and SP
#az aks create --resource-group myK8sCluster --name myK8sCluster --service-principal <appID> --client-secret <password> --node-vm-size Standard_DS3_v2 --node-count 2 --ssh-key-value ~/.ssh/id_rsa.pub -k 1.9.2
